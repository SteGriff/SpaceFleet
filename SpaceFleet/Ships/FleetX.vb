...
'    Public Sub New(ByRef Owner As Player, Location As Integer, AllShips As List(Of ShipOrgUnit))
...
'    Public Sub AssembleFleet(Owner As Player, Location As Integer, AllShips As List(Of ShipOrgUnit))
...
'        Dim Entities As New List(Of ShipOrgUnit)
...
'        'Now get the fleets
'        Dim FleetsToProcess = CandidateEntities.Where(Function(x) (TypeOf x Is FleetX)).ToList()
...
'            'Merge it
'            DirectCast(F, FleetX).MergeInto(Me, AllShips)
...
'    Public Sub MergeInto(ByRef Fleet As FleetX, AllShips As List(Of ShipOrgUnit))
